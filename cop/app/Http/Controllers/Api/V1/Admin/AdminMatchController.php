...
class AdminMatchController extends Controller
{
    ...
    {
        $id = $this->matches->softDelete($match, $request);
        return Api::success(null, "Match {$id} deleted successfully");
    }

    public function restore(Request $request, string $id): JsonResponse
    {
        $match = MatchModel::onlyTrashed()->findOrFail($id);
        return Api::success($this->matches->restore($match, $request), 'Match restored successfully');
    }
