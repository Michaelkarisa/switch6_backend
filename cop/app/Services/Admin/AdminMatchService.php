...
class AdminMatchService
{
    ...
    public function paginate(Request $request): LengthAwarePaginator
    {
        ...
        return MatchModel::with(['homeClub', 'awayClub', 'league', 'author', 'referee'])
            ->when($showDeleted, fn ($q) => $q->onlyTrashed())
            ->select([
                'id', 'league_id', 'home_club_id', 'away_club_id', 'referee_id',
                'author_id', 'match_date', 'home_score', 'away_score',
                'status', 'venue', 'home_formation', 'away_formation', 'created_at', 'deleted_at',
            ])
            ->when($request->query('status'),    fn ($q, $v) => $q->where('status', $v))
            ->when($request->query('author_id'), fn ($q, $v) => $q->where('author_id', $v))
            ->when($request->query('referee_id'), fn ($q, $v) => $q->where('referee_id', $v))
    ...
    public function restore(MatchModel $match, ?Request $request = null): MatchModel
    {
        $match->restore();

        $this->audit->log('admin_match_restored', 'matches', 'Admin restored match',
            ['match_id' => $match->id], $match, $request);

        return $match->fresh()->load(['homeClub', 'awayClub', 'author']);
    }

    public function softDelete(MatchModel $match, ?Request $request = null): string
    {
        $id = $match->id;
        $match->delete(); // soft delete

        $this->audit->log('admin_match_soft_deleted', 'matches', 'Admin soft-deleted match',
