...
class AdminPlanService
{
    ...
    public function listPlans(Request $request): \Illuminate\Contracts\Pagination\LengthAwarePaginator
    {
        return Plan::withCount('activeSubscriptions')
            ->orderBy('price')
            ->paginate((int) $request->query('per_page', 20));
    }

    public function listDeletedPlans(Request $request): \Illuminate\Contracts\Pagination\LengthAwarePaginator
    {
    ...
    public function deletePlan(Plan $plan, ?Request $request = null): void
    {
        ...
        $this->audit->log('admin_plan_deleted', 'plans', 'Admin soft-deleted plan',
            ['plan_id' => $id], null, $request);
    }

    public function restorePlan(Plan $plan, ?Request $request = null): Plan
    {
        $plan->restore();

        $this->audit->log('admin_plan_restored', 'plans', 'Admin restored plan',
            ['plan_id' => $plan->id], $plan, $request);

        return $plan->fresh();
    }

    public function forceDeletePlan(Plan $plan, ?Request $request = null): void
    {
        $id = $plan->id;
        $plan->forceDelete();

        $this->audit->log('admin_plan_force_deleted', 'plans', 'Admin permanently deleted plan',
            ['plan_id' => $id], null, $request);
    }
